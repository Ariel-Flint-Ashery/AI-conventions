...
def get_prompt(network_dict, p):
  ...
  histories = [{"role": "system", "content": rules},
        {"role": "user", "content": "You are currently playing in round 1. Your point tally is 0. Q: Which Option do you choose, Option 0 or Option 1?"},
        ]
  ...
  for idx in range(len(player['my_history'])):
    ...
    if outcome > 0: # match
      histories.append({"role": "user", "content": f"In round {idx+1}, you chose Option {my_answer} and the other player chose Option {partner_answer}. Thus you won {outcome} points. You are currently playing in round {idx+2}. Your point tally is {player['score_history'][idx]}. Q: What two character string would you like to produce from the available pool?"})

    else: # no match
      histories.append({"role": "user", "content": f"In round {idx+1}, you chose Option {my_answer} and the other player chose Option {partner_answer}. Thus you lost {outcome} points. You are currently playing in round {idx+2}. Your point tally is {player['score_history'][idx]}. Q: What two character string would you like to produce from the available pool?"})
...
fname = f'BINARY_{network_type}_{N}ps_{iterations}its_{r}rds.pkl'
f = open(fname, 'wb')
