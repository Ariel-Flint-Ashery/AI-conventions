...
1. You must produce a three character string from the pool.
2. If you and the other player produce matching stringd, then you win 10 points. For example: "In round X, you produced '{correct_example}' and the other player produced '{correct_example}'. Thus you won 10 points."
...
5. Your answer must be of the form "I will choose the string 'XXX'.".
...
initial_round = "In round 1, you produced 'fjf' and the other player produced 'jjj'. Thus you lost 5 points."
...
"""
dataframe = {it: {} for it in range(iterations)}
for it in range(iterations):
  network = {n+1: {'my_history': {}, 'partner_history': {}, 'interactions': defaultdict(int), 'score': 0, 'score_history': {}} for n in range(N)}
  simulation_dict = simulation(network, network_type = 'complete', degree = 4, rounds = r)
  dataframe[it] = simulation_dict

fname = f'{pool}_{N}ps_{iterations}its_{r}rds.pkl'
f = open(fname, 'wb')
